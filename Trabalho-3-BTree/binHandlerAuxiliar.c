...
/**
 * @brief Busca na querylist um campo em especifico e retorna seu valor string
 * 
 * @param nomeDoCampo 
 * @param list 
 * @return char* 
...
/**
 * @brief Busca na querylist um campo em especifico e retorna seu valor numerico
 * 
 * @param nomeDoCampo 
 * @param list 
 * @return char* 
...
/**
 * @brief a partir das atualizacoes, prepara um registro a ser escrito
 * 
 * @param r 
 * @param atualizacoes 
 */
void monta_registro_atualizado(Registro * r, QueryList  atualizacoes){
    ...
    if(idNascimento != EOF_TOKEN){
        r->idNascimento = idNascimento;
    }

    if(idadeMae != EOF_TOKEN){
        r->idadeMae = idadeMae;
    }
...
/**
 * @brief atualiza um registro
 * 
 * @param fp 
 * @param atualizacoes as atualizacoes a serem feitas
 * @return int um código indicando se a operação foi bem sucedida ou não
 */
int atualiza_registro(FILE *fp,QueryList  atualizacoes){
    ...
    //TODO alterar a escrita de registro depois para poder só chamar a função aqui
    fwrite(&r.cidadeMaeTamanho,sizeof(int),1,fp);
    fwrite(&r.cidadeBebeTamanho,sizeof(int),1,fp);
    for(int i=0;i<r.cidadeMaeTamanho;i++){
        fwrite(&(r.cidadeMae[i]),sizeof(char),1,fp);
